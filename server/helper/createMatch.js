  ...
  try {
    ...
    const results = candidates
      .map((candidate) => {
        ...
        const bioScore = cosineSimilarity(
          bioEmbedding.embedding.values,
          candidate.bioEmbedding || []
        );
        const moodScore = cosineSimilarity(
          moodembedding.embedding.values,
          candidate.moodembedding || []
        );
    ...
    try {
      ...
      const Macthed = new Match({
        user1: user?.email,
        user2: result?.email,
        compatibilityScore: result?.matchScore,
        isPinned: true,
        status: "matched",
      });
